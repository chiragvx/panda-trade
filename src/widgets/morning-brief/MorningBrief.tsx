...
import { RefreshCcw, Clipboard, Sparkles, Target, Zap } from 'lucide-react';
    ...
    <WidgetShell>
        <WidgetShell.Toolbar>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <span style={{ fontSize: '9px', color: COLOR.text.muted, fontWeight: TYPE.weight.bold }}>{timestamp}</span>
                <button onClick={generateBrief} disabled={loading} style={{ background: 'none', border: 'none', color: COLOR.text.muted, cursor: 'pointer', display: 'flex', alignItems: 'center' }} className="hover:text-text-primary active:opacity-50">
                    <RefreshCcw size={12} className={loading ? "animate-spin" : ""} />
                </button>
                <button onClick={copyToClipboard} style={{ background: 'none', border: 'none', color: COLOR.text.muted, cursor: 'pointer', display: 'flex', alignItems: 'center' }} className="hover:text-text-primary active:opacity-50">
                    <Clipboard size={12} />
                </button>
            </div>
        </WidgetShell.Toolbar>
        ...
        <div style={{ flex: 1, padding: SPACE[6], overflowY: 'auto' }} className="custom-scrollbar">
            {loading ? (
                <EmptyState 
                    ...
                    message="SYNTHESIZING_BRIEF"
                    subMessage="Aggregating global market data and calculating watchlist setups."
                />
            ) : brief ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: SPACE[6] }}>
                    ...
                    <div style={{ padding: SPACE[4], background: COLOR.bg.surface, border: BORDER.standard, borderLeft: `3px solid ${COLOR.semantic.info}`, marginTop: SPACE[6] }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: SPACE[4] }}>
                            <div>
                                <div style={{ fontSize: '8px', color: COLOR.text.muted, textTransform: 'uppercase', marginBottom: '2px', fontWeight: TYPE.weight.bold }}>CONVICTION_ASSET</div>
                            ...
                            <div style={{ textAlign: 'right' }}>
                                <div style={{ fontSize: '8px', color: COLOR.text.muted, textTransform: 'uppercase', marginBottom: '2px', fontWeight: TYPE.weight.bold }}>TARGET_EXIT</div>
                                <div style={{ fontSize: TYPE.size.md, fontWeight: TYPE.weight.bold, color: COLOR.semantic.info }}>N/A</div>
                            </div>
                        </div>
                        <div style={{ marginTop: SPACE[3], display: 'flex', gap: '1px', background: COLOR.bg.border }}>
                            <div style={{ flex: 1, background: COLOR.bg.base, padding: '6px', fontSize: '9px', color: COLOR.text.muted, fontWeight: TYPE.weight.bold }}>
                ...
                <EmptyState 
                    icon={<Zap size={32} />}
                    message="MORNING_BRIEF_READY"
                    subMessage="Initial market synthesis required. Trigger regeneration to build outlook."
                />
            )}
        </div>
    </WidgetShell>
