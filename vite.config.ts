  ...
  server: {
    proxy: {
      '/api/proxy/nasa-firms': {
        ...
        changeOrigin: true,
        rewrite: (path) => path.replace(/^\/api\/proxy\/nasa-firms/, '/data/active_fire/viirs-snpp-nrt/csv/VNP14IMGTDL_NRT_Global_24h.csv')
      },
      '/api/aisstream': {
        ...
        ws: true,
        secure: false,
        rewrite: (path) => path.replace(/^\/api\/aisstream/, '/v0/stream')
      },
      '/api/adsb': {
